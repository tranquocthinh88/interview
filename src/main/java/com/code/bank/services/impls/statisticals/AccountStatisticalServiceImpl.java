...
public class AccountStatisticalServiceImpl implements AccountStatisticalService {
    ...
    @Override
    public AccountStatisticalResponse getAccountsByBalanceCategory(String category) {
        List<Account> accounts;
        long count = switch (category.toLowerCase()) {
            case "low" -> {
                accounts = accountRepository.findAccountByBalanceLessThan(5000000);
                yield accountRepository.countAccountByBalanceLessThan(5000000);
            }
            case "medium" -> {
                accounts = accountRepository.findAccountByBalanceBetween(5000000, 50000000);
                yield accountRepository.countAccountByBalanceBetween(5000000, 50000000);
            }
            case "high" -> {
                accounts = accountRepository.findAccountByBalanceGreaterThan(50000000);
